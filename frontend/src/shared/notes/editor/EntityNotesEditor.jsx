...
import { getNote, upsertNote, publishNote } from "../notesApi";
import EntityNotesToolbar from "./EntityNotesToolbar";
import {
...
import {
  ...
  getPopoverPosition,
  normalizeUser,
} from "../utils/noteMentionUi";
import {
  getLastPublishedStorageKey,
  hasNewMentionsAfterPublish,
  scrollToHighlight,
...
/**
 * Generic entity-bound notes editor (identity via entityType + entityId props).
 */
export default function EntityNotesEditor({
  entityType,
  entityId,
  tableId = null,
  ...
  initialContext = null,
  onCountChange = null,
  workspaceLeftOffset = DEFAULT_WORKSPACE_LEFT_OFFSET,
  warnOnMissingPublishedRuntimeRef = true,
}) {
  ...
  const [isFullscreen, setIsFullscreen] = useState(shouldOpenFullscreenInitially);
  ...
  const fullscreenOverlayStyle = getFullscreenOverlayStyle(workspaceLeftOffset);
  ...
  }, [isLoading, isFullscreen, normalizedEntityId]);

  useEffect(() => {
    const highlightId =
      initialContext?.highlight_id || initialContext?.highlightId;

    if (!highlightId) return;

    requestAnimationFrame(() => {
      scrollToHighlight({ editorRef, highlightId });
    });
  }, [isFullscreen, normalizedEntityId, initialContext, content]);
  ...
  async function handlePublishNote() {
    if (!normalizedEntityId || isPublishing) return;

    const html = editorRef.current?.innerHTML || content || "";
    const { mentionedUserIds, mentionKeys } =
      collectMentionPayloadFromHtml(html);

    try {
      setIsPublishing(true);
      setSaveStatus("Сохранение...");

      if (warnOnMissingPublishedRuntimeRef && !publishedRuntimeRef) {
        console.warn(
          "[EntityNotesEditor] publishedRuntimeRef is missing; note_mention may not include canonical runtime reference.",
        );
      }
