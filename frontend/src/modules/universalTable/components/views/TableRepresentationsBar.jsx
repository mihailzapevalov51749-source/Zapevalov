...
export default function TableRepresentationsBar({
  ...
  useEffect(() => {
    ...
    const handleOutsideMouseDown = (event) => {
      ...
      if (isRenameEditor || isDirtyModal || isFilterModal) return;
    ...
    const handleKeyDown = (event) => {
      ...
      if (isRenameEditor && event.key !== "Escape") return;
  ...
  const renderDirtyModal = () => {
    if (!pendingRepresentation && !leavePageRequest) return null;
  ...
  const renderPinnedRepresentation = (representation, index) => {
    ...
    const isActive = String(activeRepresentationId) === representationId;

    return (
      <button
        key={`${representationId}_${index}`}
        type="button"
  ...
  return (
    <div data-table-action="true" style={tableRepresentationsRootStyle}>
        ...
        {normalizedViewsVisibleLimit > 0 && (
          <div style={tableRepresentationsPinnedGroupStyle}>
            {Array.from({ length: normalizedViewsVisibleLimit }).map(
              (_, index) =>
                renderPinnedRepresentation(pinnedRepresentations[index], index)
            )}
          </div>
        )}
      </div>
