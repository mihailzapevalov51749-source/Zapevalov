...
import TableRepresentationsBar from "../viewSettings/TableRepresentationsBar";
...
onAddRow,
  ...
  useEffect(() => {
    ...
    try {
      ...
      setFilterOrder(
        Array.isArray(parsedValue) ? parsedValue.map(String) : []
      );
    } catch {
      setFilterOrder([]);
    }
...
const handleFilterClick = (filter) => {
  ...
  <button
    ...
    style={getToolButtonStyle(isInlineEditMode)}
    title={
      isInlineEditMode
  ...
  >
    Фильтры
      ...
      <div style={representationsAreaStyle}>
        <TableRepresentationsBar {...representationsBarProps} />
      </div>
    </div>
  );
}
...
const representationsAreaStyle = {
  display: "flex",
  alignItems: "center",
...
const getFilterStyle = ({
  ...
  transition:
    "border 0.12s ease, background 0.12s ease, opacity 0.12s ease",
});
...
const getToolButtonStyle = (active) => ({
  ...toolButtonStyle,
