...
export default function ObjectEntityRelatedEntities({
  ...
  const hasHierarchyGroups = hierarchyChildGroups.length > 0;
  const hasRegularGroups = regularGroups.length > 0;
  const hasAnyGroups = hasHierarchyGroups || hasRegularGroups;
  ...
  return (
    <div style={entityCardSubtasksStyle}>
      ...
      {!loading && !error && !hasAnyGroups ? <EmptyState /> : null}

      {!loading && !error && hasHierarchyGroups
        ? hierarchyChildGroups.map((group) => (
            <HierarchyChildRelationsGroup
              key={`hierarchy-${group.relationKey}-${group.direction}`}
              group={group}
              tenantId={tenantId}
              entityId={entityId}
              objectTypeKey={currentObjectTypeKey}
              catalog={catalog}
              canAddChild={canCreateHierarchyChild}
              creating={creating}
              deletingInstanceId={deletingInstanceId}
              mutationError=""
              onOpenRelatedEntity={onOpenRelatedEntity}
              onCreateNewChild={() => onBeginCreateHierarchyChild?.(group.relationKey)}
              onLinkExistingChild={async (childEntityId) =>
                onCreateRelation?.({
                  relationKey: group.relationKey,
                  currentRole: "source",
                  peerEntityId: childEntityId,
                })
              }
              onUnlinkChild={onDeleteRelation}
            />
          ))
        : null}

      {!loading && !error && hasRegularGroups ? (
        <>
