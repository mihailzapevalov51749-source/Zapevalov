...
export default function useObjectEntityRelations({
  ...
  const [hierarchyChildGroups, setHierarchyChildGroups] = useState([]);
  ...
  useEffect(() => {
    ...
    async function loadRelations() {
      ...
      try {
        ...
        const mapped = await mapRelationInstancesToGroups({
          instances,
        ...
        if (!cancelled) {
          setHierarchyChildGroups(mapped.hierarchyChildGroups);
          setRegularGroups(mapped.regularGroups);
        }
      } catch (err) {
        if (!cancelled) {
          setHierarchyChildGroups([]);
  ...
  const primaryHierarchyRelationKey = useMemo(
    () => resolvePrimaryHierarchySubtaskRelationKey(catalog, normalizedObjectTypeKey),
    [catalog, normalizedObjectTypeKey],
  );

  const hierarchyFeatureEnabled = useMemo(
    () => hasHierarchySubtasksFeature(catalog, normalizedObjectTypeKey),
    [catalog, normalizedObjectTypeKey],
  );

  const hierarchyChildGroupsForDisplay = useMemo(() => {
    if (hierarchyChildGroups.length) {
      return hierarchyChildGroups;
    }

    if (!hierarchyFeatureEnabled || !primaryHierarchyRelationKey) {
      return [];
    }
    ...
    const catalogRelation =
      relations.find(
        (relation) => String(relation?.key || "").trim() === primaryHierarchyRelationKey,
      ) || { key: primaryHierarchyRelationKey };

    const uiLabels = resolveHierarchyChildUiLabels(catalogRelation);

    return [
      {
        relationKey: primaryHierarchyRelationKey,
        title: uiLabels.groupTitle,
        direction: "outgoing",
        relationType: "",
    ...
    hierarchyChildGroups,
    hierarchyFeatureEnabled,
    primaryHierarchyRelationKey,
    catalog,
  ]);

  const totalCount = useMemo(() => {
    const countGroups = (items) =>
      items.reduce((total, group) => total + (group.items?.length || 0), 0);

    return countGroups(hierarchyChildGroupsForDisplay) + countGroups(regularGroups);
  }, [hierarchyChildGroupsForDisplay, regularGroups]);

  return {
    loading,
    error,
    hierarchyChildGroups: hierarchyChildGroupsForDisplay,
    regularGroups,
    primaryHierarchyRelationKey,
    hierarchyFeatureEnabled,
    totalCount,
