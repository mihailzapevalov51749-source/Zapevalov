#!/usr/bin/env python3
"""Record Plan inline editing audit fix in istoriya and napravleniya plan tree."""
...
STEP_TITLE = "Аудит Plan Inline Editing — регресс frontend"
STATUS_DONE_LABEL = "Готово"
STATUS_NOT_STARTED_LABEL = "Не начато"

HISTORY_TITLE = "Аудит Plan Inline Editing — устранён регресс запуска frontend"
HISTORY_DESCRIPTION = (
    "Тип события: Реализация. Область: Plan View. "
    "Проведён аудит реализации Plan Inline Editing. "
    "Устранён регресс запуска frontend: FieldValueRenderer импортируется из "
    "shared/fieldTypes/FieldValueRenderer (как ViewEngineCell), "
    "а не из несуществующего shared/fieldEditors/FieldValueRenderer. "
    "PlanInlineFieldCell выровнен с платформенной архитектурой "
    "(FieldEditor + FieldValueRenderer + fieldDefToRendererColumn). "
    "Изменённые файлы: frontend/src/modules/objectViews/plan/PlanInlineFieldCell.jsx, "
    "frontend/src/modules/platformDashboard/data/platformDevelopmentManifest.js. "
    "Автор: Cursor."
)
HISTORY_RESULT = (
    "Frontend собирается без ошибок import-analysis; "
    "Plan и Table используют единый persistRuntimeEntityFieldUpdate pipeline."
)
...
def main() -> int:
    ...
    try:
        ...
        if existing_step_id is None:
            ...
            _link_parent_child(db, root_id, step_id)
